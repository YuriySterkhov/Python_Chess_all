...
class ChessGame(QMainWindow):
    def __init__(self):
        ...
        # Шахматная доска
        self.board = chess.Board('8/8/8/8/8/8/5KR1/7k w - - 0 1')
    ...
    def update_animation(self):
        ...
        if self.step == 0:
            ...
            self.board.set_piece_at(chess.G2, chess.Piece(chess.ROOK, chess.WHITE))
            self.board.set_piece_at(chess.H1, chess.Piece(chess.KING, chess.BLACK))

            # Получаем позицию белого короля на f2
            rook_square = chess.F2

            # Генерация возможных ходов для белого короля на f2
            all_moves = self.board.legal_moves
            white_king_moves_squares = chess.SquareSet()

            # Создание набора клеток для выделения
            for move in all_moves:
                if move.from_square == rook_square:
                    white_king_moves_squares.add(move.to_square)

             # Генерация SVG с выделением
            svg = chess.svg.board(
                board=self.board,
                squares=white_king_moves_squares,
            )
